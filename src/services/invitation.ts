...
export const InvitationService = {
  async send(email: string, invitedBy: string): Promise<Invitation> {
    ...
    await resend.emails.send({
      from: env.fromEmail ?? 'noreply@example.com',
      to: email,
  ...
  async resend(id: string, updatedBy: string): Promise<Invitation> {
    const invitation = await InvitationRepository.updateStatus(id, 'resent', updatedBy);
  ...
  async accept(token: string, userId: string): Promise<Invitation> {
    ...
    const updated = await InvitationRepository.updateStatus(invitation.id, 'accepted', userId);
    if (!updated) throw new Error('Failed to accept invitation');
    return updated;
  },

  async cancel(id: string, updatedBy: string): Promise<Invitation> {
    const invitation = await InvitationRepository.updateStatus(id, 'cancelled', updatedBy);
